...
public class TelaRedeSocial {
    ...
    private void curtirMensagem() {
        if (feed.nroMensagens()==0){
            throw new RuntimeException("Não há mensagens para curtir.");
        }
        try {
            int idMensagem = Integer.parseInt(JOptionPane.showInputDialog("Id da mensagem"));
            if (feed.curtir(idMensagem)){
                atualizarAreaTextoFeed();
            }
            else {            
                JOptionPane.showMessageDialog(janela, "Digite um id de mensagem válido.", "Erro", JOptionPane.ERROR_MESSAGE);
            }
    ...
    private void comentarMensagem() {
        if (feed.nroMensagens()==0){
            throw new RuntimeException("Não há mensagens para comentar.");
        }
        try {
            int idMensagem = Integer.parseInt(JOptionPane.showInputDialog("Id da mensagem"));
            ...
            try{
                ...
                JOptionPane.showMessageDialog(janela, "A mensagem de id " + ex.getId() + " não foi encontrada.", "Erro", JOptionPane.ERROR_MESSAGE);
                comentarMensagem();
            }
            atualizarAreaTextoFeed();
        } catch (NumberFormatException ex) {
